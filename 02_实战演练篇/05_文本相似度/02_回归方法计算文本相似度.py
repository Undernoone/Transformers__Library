...
Description: 使用回归方法训练文本相似度，因为很多时候不是一对一的相似度匹配，而是一对多的相似度匹配，
             因此这就相当于多项选择，把一当作问题，多当作选项，如果使用分类方法，所有相似的都是1，
             而不相似的都是0，无法区分哪个选项是最相似的，因此需要用回归方法。
             回归方法可以直接预测一个连续值，因此可以直接用预测的结果来衡量文本相似度。
"""
...
def preprocess_function(examples):
    ...
    tokenized_examples["label"] = [float(label) for label in examples["label"]] # 神经网络训练时不能用字符串，所以需要转换为整数,这里要做回归任务，所以label字段需要转换为float
